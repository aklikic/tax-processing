...
public class BatchControllerWorkflowPostgreSQLIntegrationTest extends TestKitSupport {
    ...
    public void shouldProcessSmallBatchWithPostgreSQL() throws Exception {
        ...
        var startCommand = new BatchControllerWorkflow.StartBatchCommand(batchId, taxYear);
        ...
        Awaitility.await()
            ...
            .untilAsserted(() -> {
                ...
                System.out.println("Status: " + status.status() + ", Total: " + status.totalPositions() +
                                 ", Completed: " + status.completedPositions() + ", Windows: " + status.totalWindows());
                ...
                assertThat(status.totalWindows()).isEqualTo(1); // 3 positions with window size 3 = 1 window
    ...
    public void shouldProcessMultipleWindowsWithPostgreSQL() throws Exception {
        ...
        Awaitility.await()
            ...
            .untilAsserted(() -> {
                ...
                System.out.println("Multi-window Status: " + status.status() +
                                 ", Total: " + status.totalPositions() +
                                 ", Completed: " + status.completedPositions() +
                                 ", Windows: " + status.totalWindows());

                assertThat(status.status()).isEqualTo(
                    BatchControllerState.ProcessingStatus.COMPLETED
                );
                assertThat(status.totalPositions()).isEqualTo(5);
                assertThat(status.totalWindows()).isEqualTo(2); // 5 positions with window size 3 = 2 windows
            });
